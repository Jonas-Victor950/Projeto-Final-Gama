...
const ProfissionalServicoController = {
  ...
  async listarProfissionalServico(req: Request, res: Response) {
    try {
        const profissionalServicos = await ProfissionalServicoRepository.listarProfissionalServico()
...
async listarProfissionalServicoId(req: Request, res: Response) {
  try {
      ...
      const profissionalServicoId = await ProfissionalServicoRepository.listarProfissionalServicoId(id)
...
async atualizarProfissionalServico(req: Request, res: Response) {
  try {
      ...
      const newProfissionalServico: { profissional: Types.ObjectId,
        servico: Types.ObjectId} = req.body;
